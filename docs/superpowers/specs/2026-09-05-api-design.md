...
- **치료사 링크**: 케이스당 활성 링크 하나. UUID, 해시 저장. `GET /t/{token}`은 헤더 없이 읽기 전용. 폐기하면 404. 해시만 저장하므로 기존 링크의 평문을 되돌려줄 수 없다. 그래서 발급은 `POST`이고, **호출마다 새 링크를 만들고 이전 활성 링크를 폐기한다.** 보호자가 새로 뽑으면 옛 QR은 죽는다.
...
- `therapistGlance`: `SUSTAINED` 판정이 있는 항목마다 한 줄. 한 항목에 여러 축이 있으면 같은 줄에 쉼표로 잇는다. 형식은 `"{항목 라벨}: {이전} → {현재} ({since}주차부터)"`, 축이 level이 아니면 값 앞에 `"{축 라벨}: "`. 정렬은 선택된 질문이 참조하는 항목을 먼저 두고(카탈로그 순서), 나머지 항목을 그다음에 이어 붙인 뒤(역시 카탈로그 순서) 최대 4줄로 자른다 — 그러지 않으면 질문이 실제로 가리키는 소견이 잘려 나갈 수 있다. 판정 단어 없이 값만.
- `PUT /me/prep-card/extra { "questions": ["…"] }` → 보호자 추가 질문 전체 교체. 길이 각 200자, 최대 5개.

### `POST /me/therapist-link`

`{ "url": "/t/{token}", "token": "…" }`. 새 링크를 발급하고 이전 활성 링크를 폐기한다(4절). 프론트가 QR·인쇄로 만든다.
