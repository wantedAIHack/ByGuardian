...
- [ ] **Step 10: 카탈로그 계약을 백엔드 테스트로 고정한다**

프론트의 `fixtures.ts`는 지금까지 백엔드 **소스를 읽어** 맞춘 것이다. 살아 있는 응답과 맞춰본 적이 없다.

원래 계획은 `bootRun`을 띄우고 `curl`로 대조하는 것이었는데 **이 환경에서는 안 된다.**
`backend/api/build.gradle.kts`가 H2를 `testRuntimeOnly`로 두고 있어 `bootRun`은 Postgres를 요구하고,
도커 이미지 받기가 이 샌드박스에서 멈춘다.

한 번 맞춰보는 `curl`보다 나은 방법이 있다. **백엔드 테스트에 계약을 고정한다.**
`DemoFlowTest`는 실제 Spring 컨텍스트에서 MockMvc로 `/catalog`를 때리므로 살아 있는 응답과 같고,
도커가 필요 없으며, 카탈로그가 어긋나는 날 **자동으로 실패한다.**

`backend/api/src/test/java/nextvisit/api/demo/DemoFlowTest.java`의
`catalogExposesSetLabelsAndEnums`를 프론트 표본이 의존하는 값 전부로 넓힌다.
...
`java.util.ArrayList`와 `java.util.List` import를 확인한다.
...
./gradlew :api:test --tests '*DemoFlowTest*'
```

한 줄이라도 실패하면 **서버가 이긴다.** `frontend/src/test/fixtures.ts`를 고치고, 그 값을 쓰던
프론트 테스트도 함께 고친다.

- [ ] **Step 10b: 사람이 브라우저에서 확인할 목록을 남긴다**

브라우저가 없어 여기서는 못 돈다. `docs/manual-check.md`에 아래를 적어 두고, 배포 전에 사람이 훑는다.
