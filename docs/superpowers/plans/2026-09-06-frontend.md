...
describe('catalog', () => {
  it('코드로 항목을 찾는다', () => {
    expect(itemByCode(c, 'transfer')?.label).toBe('옮겨 앉기');
...
export const catalogFixture: Catalog = {
  set: 'stroke',
  items: [
    { code: 'transfer', label: '옮겨 앉기', phrase: '옮겨 앉기', group: 'mobility', axes: ['LEVEL', 'AID', 'CONSISTENCY'] },
    { code: 'indoor_walk', label: '집 안에서 걷기', phrase: '집 안에서 걷기', group: 'mobility', axes: ['LEVEL', 'AID', 'CONSISTENCY'] },
    { code: 'toilet', label: '화장실 이용', phrase: '화장실 이용', group: 'adl', axes: ['LEVEL', 'CONSISTENCY'] },
    { code: 'dressing', label: '옷 입기', phrase: '옷 입기', group: 'adl', axes: ['LEVEL', 'CONSISTENCY', 'HAND'] },
    { code: 'eating', label: '식사하기', phrase: '식사하기', group: 'adl', axes: ['LEVEL', 'CONSISTENCY', 'HAND'] },
    { code: 'bathing', label: '씻기', phrase: '씻기', group: 'adl', axes: ['LEVEL', 'CONSISTENCY'] },
    { code: 'stairs', label: '계단 오르내리기', phrase: '계단 오르내리기', group: 'mobility', axes: ['LEVEL', 'AID', 'CONSISTENCY'] },
    { code: 'outdoor_walk', label: '집 밖에서 걷기', phrase: '집 밖에서 걷기', group: 'mobility', axes: ['LEVEL', 'AID', 'CONSISTENCY'] },
  ],
  axes: {
    LEVEL: [
  ...
  signalActions: [
    ...
    { code: 'WALKING', label: '걸을 때' }, { code: 'BATHING', label: '씻을 때' },
    { code: 'EATING', label: '드실 때' }, { code: 'REST', label: '쉬실 때' },
  ],
  signalKinds: [
    { code: 'GRIMACE', label: '찡그림' }, { code: 'GUARDING', label: '피함' },
    { code: 'VOCALIZE', label: '소리 냄' }, { code: 'STIFFEN', label: '굳음' },
  ],
  timeTags: [
    { code: 'MORNING', label: '아침' }, { code: 'AFTERNOON', label: '낮' },
    { code: 'EVENING', label: '저녁' }, { code: 'NIGHT', label: '밤' },
  ],
  sleepLevels: [
...
describe('onboarding state', () => {
  ...
  it('7단계부터 항목이 카탈로그 순서로 하나씩 나온다', () => {
    ...
    expect(itemForStep(c, FIRST_BASELINE_STEP + 7)?.code).toBe('outdoor_walk');
  ...
  it('적용되는 축을 다 채워야 항목 화면을 넘어간다', () => {
    const s = initialState();
    s.pareticSide = 'LEFT';
    s.step = FIRST_BASELINE_STEP + 3; // dressing: LEVEL, CONSISTENCY, HAND
    expect(canAdvance(c, s)).toBe(false);
...
describe('온보딩 기준선', () => {
  it('항목 이름과 도움 수준 넷을 보여준다', () => {
    seed();
    renderScreen();
    expect(screen.getByText('옮겨 앉기')).toBeInTheDocument();
  ...
  it('마비 쪽을 모르면 마비 쪽 손을 묻지 않는다', async () => {
    const user = userEvent.setup();
    // dressing(4번째 항목, 10단계)은 HAND 축이 있다
    seed({ step: 10, pareticSide: 'UNKNOWN' });
    renderScreen();
...
describe('steps', () => {
  ...
  it('고른 항목만큼 화면이 늘어난다', () => {
    const s = { ...initialRecord(), noChange: false, selected: ['toilet', 'indoor_walk'] };
...
describe('주간 기록', () => {
  ...
  it('전체 재확인 주에 지난달 답을 함께 보여준다', async () => {
    const user = userEvent.setup();
    renderRecord(me({ fullRecheck: true, week: 8 }), [{
      code: 'transfer', label: '옮겨 앉기', changed: true,
      axes: [{
        axis: 'LEVEL', axisLabel: '도움 수준',
...
const data: T[] = [
  ...
  {
    code: 'bathing', label: '씻기', changed: false,
    axes: [{
      axis: 'LEVEL', axisLabel: '도움 수준',
...
describe('전체 궤적', () => {
  ...
  it('바뀐 것이 없는 항목은 접어둔다', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /씻기/ }));
...
const full: Card = {
  ...
  questions: [{
    ...
    evidence: {
      items: [{
        code: 'indoor_walk', label: '집 안에서 걷기', axis: 'LEVEL', axisLabel: '도움 수준',
...
`Collapse`의 버튼 이름이 `씻기 — 바뀐 것 없음`이 되므로 테스트의 `{ name: /씻기/ }`가 맞는다.
