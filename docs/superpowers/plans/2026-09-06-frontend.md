...
- **보호자에게 무언가를 보여주는 모든 화면의 모든 도달 가능한 상태가 화이트리스트로 덮여야 한다.**
  "화면당 하나"가 아니다. 조건부로만 렌더되는 분기 — 질문이 있는 준비 카드, 재발급된 코드 화면 —
  는 기본 상태에 건 화이트리스트가 **구조적으로 볼 수 없다.** 그리고 위험한 문구가 앉을 자리는
  대개 그 조건부 분기 쪽이다. 준비 카드에서 두 번, 설정에서 한 번, 치료사 페이지에서 한 번 났다.
- **어느 분기가 위험한지 가리는 기준: else 쪽이 글자를 렌더하는가.**
  `x.length > 0 ? (...) : null`처럼 else가 `null`이면 조작될 자리가 없다.
  `x.length === 0 ? <p>관찰된 신호 없음</p> : (...)`처럼 else가 하드코딩 문구를 내면 위험하다.
  네 번의 결함이 전부 후자였다. "하드코딩이라 데이터가 아니니 안전하다"는 정확히 거꾸로 된 판단이다 —
  하드코딩 문구야말로 지어낸 문장이 앉기 좋은 자리이고, 표본을 바꿔도 드러나지 않는다.
  서버가 판정 필드를 주지 않으므로 판정 문구는 지어내야만 생기고, 지어낸 문구는 어느 화면에서든
  들어올 수 있다. "이 화면은 판정을 안 만든다"는 코드를 읽어서가 아니라 조작된 문구를 넣어봐서
  확인한다. 확인 방법: 문구 하나를 심어 실패하는지 보고, **다른 문구로도** 실패하는지 본다.
  한 문구만 잡는 화이트리스트는 위장한 블랙리스트다.
...
export function Record({ catalog }: { catalog: Catalog }) {
  ...
  // **훅이므로 아래의 조기 반환보다 반드시 위에 둔다.** 아래에 두면 첫 렌더(me가 아직 없음)에서
  // 호출되지 않았다가 다음 렌더에서 호출돼 "Rendered more hooks than during the previous render"로
  // 화면이 죽는다.
