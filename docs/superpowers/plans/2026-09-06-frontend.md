...
**Files:**
- Create: `frontend/src/lib/constants.ts`, `frontend/src/lib/catalog.ts`, `frontend/src/lib/format.ts`, `frontend/src/lib/draft.ts`, `frontend/src/lib/queries.ts`
...
- Produces:
  ...
  - `catalog.ts`: `CatalogProvider`, `useCatalog(): Catalog`, `itemByCode(c, code)`, `axisValues(c, axis)`, `axisValueLabel(c, axis, value)`, `axisName(c, axis)`, `codeLabel(list, code)`
...
catalog.ts는 순수 조회 함수와 React 배선을 갈라 두었다. 조회는 화면 없이
...
**Interfaces:**
- Consumes: Task 3의 `constants.ts` 선택지 셋, `draft.ts`의 `ONBOARDING_DRAFT`·`loadDraft`·`saveDraft`·`clearDraft`, `catalog.ts`의 `useCatalog`·`itemByCode`, `ui/` 부품 전부.
- Produces:
...
**Interfaces:**
- Consumes: Task 4의 `lib/onboarding.ts` 전부, Task 3의 `catalog.ts` 조회 함수·`api`·`setToken`·`ApiError`.
- Produces:
...
**Interfaces:**
- Consumes: Task 3의 `queries.ts`(`useMe`, `useTrajectory`, `useSaveWeek`), `catalog.ts` 조회 함수, `draft.ts`의 `weeklyDraftKey`, `api.ts`의 `ApiError`.
- Produces:
...
describe('설정', () => {
  it('초대는 지금 있는 코드를 알려주는 흐름이라고 말한다', async () => {
    renderIt();
    await screen.findByText('다른 가족 초대하기');
    expect(screen.getByText(/지금 갖고 계신 이어받기 코드를 알려주시면 됩니다/)).toBeInTheDocument();
    // 초대 안내 안에 재발급 버튼이 들어가면 안 된다
    expect(screen.queryByRole('button', { name: '코드 새로 만들기' })).toBeInTheDocument();
  });
...
`routes.tsx`의 `demo` 항목을 `<Demo />`로 바꾼다.
